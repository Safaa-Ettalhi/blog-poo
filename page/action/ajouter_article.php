...
try {
    ...
    $stmt_tags = $conn->query($query_tags);
    $result_tags = $stmt_tags->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tags_id = isset($_POST['tags']) ? intval($_POST['tags']) : null;
    $url_image = trim($_POST['thumbnail_url']);
    ...
    if (empty($titre) || empty($contenu)) {
        ...
        try {
            ...
            $article = new Article(null, $titre, $contenu, $url_image, $utilisateur_id);
            $article->creer($conn);
            ...
            if ($tags_id) {
                $stmt_tags = $conn->prepare(
                    "INSERT INTO article_tags (article_id, tags_id) 
                     VALUES (:article_id, :tags_id)"
                );
...
<body class="mx-4 mx-8">
    ...
    <section class="bg-white border-2 border-[#cb6ce6] rounded-lg shadow-lg p-8 mt-10">
        <form method="POST" class="space-y-6">
            ...
            <div id="editor-container" style="border: 1px solid ; border-radius: 4px; min-height: 700px;"></div>
    ...
    placeholder: 'Write the full content of the article here...',
            ...
            <div>
                ...
                <input type="text" id="thumbnail" name="thumbnail_url"
                    class="w-full px-4 py-3 border-2 border-[#cb6ce6] bg-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#cb6ce6] placeholder-gray-400"
                    placeholder="Enter image URL for the thumbnail (optional)">
            </div>
